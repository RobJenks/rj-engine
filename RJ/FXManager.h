...
#ifndef __FXManagerH__
#define __FXManagerH__
/*
#include <windows.h>
#include <xnamath.h>
#include <d3d11.h>
...
public:
	...
	void SetMaterial(const SkinnedModel::SM_Material& mat)	{ Mat->SetRawValue(&mat, 0, sizeof(SkinnedModel::SM_Material)); }
...
{
public:
	NormalMapEffect(ID3D11Device* device, const std::string& filename);
	~NormalMapEffect();
...
{
public:
	BuildShadowMapEffect(ID3D11Device* device, const std::string& filename);
...
{
public:
	SsaoNormalDepthEffect(ID3D11Device* device, const std::string& filename);
...
{
public:
	SsaoEffect(ID3D11Device* device, const std::string& filename);
	~SsaoEffect();
...
{
public:
	SsaoBlurEffect(ID3D11Device* device, const std::string& filename);
	~SsaoBlurEffect();
...
{
public:
	DebugTexEffect(ID3D11Device* device, const std::string& filename);
	~DebugTexEffect();
...
{
public:
	FXManager(DXLocaliser *localiser);
	~FXManager(void);
