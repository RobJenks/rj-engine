...
Result TextureShader::Initialise(Rendering::RenderDeviceType * device, HWND hwnd)
{
	...
	result = InitialiseVertexShader(device, iShader::ShaderFilename("texture.vs.cso"));
	if (result != ErrorCodes::NoError) return result;

	result = InitialisePixelShader(device, iShader::ShaderFilename("texture.ps.cso"));
	if (result != ErrorCodes::NoError) return result;
...
Result TextureShader::InitialiseVertexShader(Rendering::RenderDeviceType  *device, std::string filename)
{
	...
	// Parameter check
	if (!device || filename == NullString) return ErrorCodes::NullInputToCreateTextureShaderVS;
	...
	if (result != ErrorCodes::NoError) return ErrorCodes::ErrorCreatingTextureShaderVS;
	...
	if (result != ErrorCodes::NoError) return ErrorCodes::ErrorCreatingTextureShaderConstBuffersVS;
...
Result TextureShader::InitialisePixelShader(Rendering::RenderDeviceType  *device, std::string filename)
{
	...
	// Parameter check
	if (!device || filename == NullString) return ErrorCodes::NullInputToCreateTextureShaderPS;

	// Attempt to load and create the compiled shader 
	result = ShaderManager::CreatePixelShader(device, filename, &m_pixelShader);
	if (result != ErrorCodes::NoError) return ErrorCodes::ErrorCreatingTextureShaderPS;
	...
	if (result != ErrorCodes::NoError) return ErrorCodes::ErrorCreatingTextureShaderSamplerPS;
