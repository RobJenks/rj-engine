...
Result D3DMain::Initialise(int screenWidth, int screenHeight, bool vsync, HWND hwnd, bool fullscreen, 
						   float screenDepth, float screenNear)
{
	...
	if(error != 0)
	{
		return ErrorCodes::CouldNotConvertAndStoreAdapterDescData;
	}
	...
	if(FAILED(result))
	{
		return ErrorCodes::CouldNotCreateD3DDeviceAndSwapChain;
	}
	...
	if(FAILED(result))
	{
		return ErrorCodes::CouldNotObtainPointerToBackBuffer;
	}
	...
	if(FAILED(result))
	{
		return ErrorCodes::CouldNotCreateRenderTargetView;
	}
	...
	if(FAILED(result))
	{
		return ErrorCodes::CouldNotCreateDepthBufferTextture;
	}
	...
	if(FAILED(result))
	{
		return ErrorCodes::CouldNotCreateDepthStencilState;
	}

	// Set the depth stencil state.
	m_deviceContext->OMSetDepthStencilState(m_depthStencilState, 1);

	// Create an additional depth stencil state, with no depth writing allowed, for some alpha blending operations
	depthStencilDesc.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ZERO;
	result = m_device->CreateDepthStencilState(&depthStencilDesc, &m_depthStencilStateNoDepthWrite);
	if(FAILED(result))
	{
		return ErrorCodes::CouldNotCreateDepthStencilState;
	}
	...
	if(FAILED(result))
	{
		return ErrorCodes::CouldNotCreateDepthStencilView;
	}
	...
	rasterDesc.FillMode = m_fillmode = D3D11_FILL_SOLID;
	...
	if(FAILED(result))
	{
		return ErrorCodes::CouldNotCreateRasteriserState;
	}

	// Now set the rasterizer state.
	m_deviceContext->RSSetState(m_rasterState);
	
	// Create an additional raster state with culling enabled, for e.g. rendering the inside of the skybox
	...
	if(FAILED(result))
	{
		return ErrorCodes::CouldNotCreateDepthDisabledStencilView;
	}

	// Sent blend state factor for alpha blending
	m_alphablendfactor[0] = 0.0f; m_alphablendfactor[1] = 0.0f; 
	m_alphablendfactor[2] = 0.0f; m_alphablendfactor[3] = 0.0f;
	...
	if(FAILED(result))
	{
		return ErrorCodes::CouldNotCreateAlphaEnabledBlendState;
	}
	...
	if(FAILED(result))
	{
		return ErrorCodes::CouldNotCreateAlphaDisabledBlendState;
	}

	// Modify the description to create an additive alpha blend state (for effects etc)
	blendStateDescription.RenderTarget[0].BlendEnable = TRUE;
	...
	result = m_device->CreateBlendState(&blendStateDescription, &m_alphaEnableAdditiveBlendingState);
	if(FAILED(result))
	{
		return ErrorCodes::CouldNotCreateAlphaEnabledAdditiveBlendState;
	}
...
void D3DMain::SetFillMode(D3D11_FILL_MODE fillmode)
{
	...
	if (fillmode != m_fillmode)
	{
		...
		// Change the fill mode within this rasteriser description, and also the tracking member variable
		rdesc.FillMode = m_fillmode = fillmode;

		// Create a new rasteriser state from this new description and apply it to the DX device
...
void D3DMain::BeginScene()
{
	// Clear the back buffer.
	m_deviceContext->ClearRenderTargetView(m_renderTargetView, m_bufferbg);
...
void D3DMain::EnableZBuffer(void)
{
	// Enable z-buffering for normal 3D rendering
	m_deviceContext->OMSetDepthStencilState(m_depthStencilState, 1);
}

void D3DMain::DisableZBuffer(void)
{
