...
DXGI_FORMAT TextureDX11Util::TranslateFormat(const Texture::TextureFormat & format)
{
...
DXGI_FORMAT TextureDX11Util::GetTextureFormat(DXGI_FORMAT format)
{
...
DXGI_FORMAT TextureDX11Util::GetDSVFormat(DXGI_FORMAT format)
{
	DXGI_FORMAT result = format;
...
DXGI_FORMAT TextureDX11Util::GetSRVFormat(DXGI_FORMAT format)
{
	DXGI_FORMAT result = format;
	switch (format)
	{
...
DXGI_FORMAT TextureDX11Util::GetRTVFormat(DXGI_FORMAT format)
{
	DXGI_FORMAT result = format;
...
DXGI_FORMAT TextureDX11Util::GetUAVFormat(DXGI_FORMAT format)
{
	return GetRTVFormat(format);
}

uint8_t TextureDX11Util::GetBPP(DXGI_FORMAT format)
{
	uint8_t bpp = 0;
	...
	case DXGI_FORMAT_B8G8R8X8_UNORM_SRGB:
		...
		break;
	default:
		Game::Log << LOG_ERROR << ("Unsupported texture format, cannot determine BPP.  Returning default 8bpp but failure expected");
		break;
	}
...
Texture::TextureFormat TextureDX11Util::TranslateFormat(DXGI_FORMAT format, uint8_t numSamples)
{
	...
	default:
		Game::Log << LOG_ERROR << ("Unsupported DXGI format.  Returning default format but failure expected");
		result.Components = Texture::Components::RGBA;
		result.Type = Texture::Type::Typeless;
		result.RedBits = 8;
		result.GreenBits = 8;
...
DXGI_SAMPLE_DESC TextureDX11Util::GetSupportedSampleCount(DXGI_FORMAT format, uint8_t numSamples)
{
