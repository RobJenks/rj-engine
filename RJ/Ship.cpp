...
// Calculates the maximum potential thrust that can be generated by this ship's engines
float Ship::GetMaximumPossibleEngineThrust(void)
{
	// Get a reference to the set of engines on this ship
	Hardpoints::HardpointCollection & engines = GetHardpoints().GetHardpointsOfType(Equip::Class::Engine);

	// Accumulate the maximum possible thrust from each engine hardpoint
	float max_thrust = 0.0f;
	Hardpoints::HardpointCollection::const_iterator it_end = engines.end();
	for (Hardpoints::HardpointCollection::const_iterator it = engines.begin(); it != it_end; ++it)
		max_thrust += ((HpEngine*)*it)->GetMaxPossibleThrust();

	return max_thrust;
}


// Calculates the maximum potential reverse thrust that can be generated by this ship's engines
float Ship::GetMaximumPossibleReverseEngineThrust(void)
{
	// Get a reference to the set of engines on this ship
	Hardpoints::HardpointCollection & engines = GetHardpoints().GetHardpointsOfType(Equip::Class::Engine);
	...
	Hardpoints::HardpointCollection::const_iterator it_end = engines.end();
	for (Hardpoints::HardpointCollection::const_iterator it = engines.begin(); it != it_end; ++it)
		max_thrust += ((HpEngine*)*it)->GetMinPossibleThrust();
...
void Ship::ProcessDebugCommand(GameConsoleCommand & command)
{
	...
	REGISTER_DEBUG_ACCESSOR_FN(GetMaximumPossibleEngineThrust)
	REGISTER_DEBUG_ACCESSOR_FN(GetMaximumPossibleReverseEngineThrust)
