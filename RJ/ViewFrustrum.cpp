...
ViewFrustrum::ViewFrustrum(void)
{
	...
	for (int i = 0; i < 6; ++i) m_planes[i] = XMVectorZero();
...
void ViewFrustrum::ConstructFrustrum(const FXMMATRIX view, const CXMMATRIX invview)
{
	...
	m_planes[0] = XMPlaneNormalize(XMVectorSet(	matrix._14 + matrix._13,
												matrix._24 + matrix._23, 
												matrix._34 + matrix._33, 
												matrix._44 + matrix._43));

	// Calculate far plane of frustum.
	m_planes[1] = XMPlaneNormalize(XMVectorSet(	matrix._14 - matrix._13,
												matrix._24 - matrix._23,
												matrix._34 - matrix._33,
												matrix._44 - matrix._43));

	// Calculate left plane of frustum.
	m_planes[2] = XMPlaneNormalize(XMVectorSet(	matrix._14 + matrix._11,
												matrix._24 + matrix._21,
												matrix._34 + matrix._31,
												matrix._44 + matrix._41));

	// Calculate right plane of frustum.
	m_planes[3] = XMPlaneNormalize(XMVectorSet(	matrix._14 - matrix._11,
												matrix._24 - matrix._21,
												matrix._34 - matrix._31,
												matrix._44 - matrix._41));

	// Calculate top plane of frustum.
	m_planes[4] = XMPlaneNormalize(XMVectorSet(	matrix._14 - matrix._12,
												matrix._24 - matrix._22,
												matrix._34 - matrix._32,
												matrix._44 - matrix._42));

	// Calculate bottom plane of frustum.
	m_planes[5] = XMPlaneNormalize(XMVectorSet(	matrix._14 + matrix._12,
												matrix._24 + matrix._22,
												matrix._34 + matrix._32,
												matrix._44 + matrix._42));
...
bool ViewFrustrum::CheckRectangle(float xCenter, float yCenter, float zCenter, float xSize, float ySize, float zSize)
{
	...
	m_working_cuboidvertices[0] = XMVectorSet((xCenter - xSize), (yCenter - ySize), (zCenter - zSize), 0.0f);
	m_working_cuboidvertices[1] = XMVectorSet((xCenter + xSize), (yCenter - ySize), (zCenter - zSize), 0.0f);
	m_working_cuboidvertices[2] = XMVectorSet((xCenter - xSize), (yCenter + ySize), (zCenter - zSize), 0.0f);
	m_working_cuboidvertices[3] = XMVectorSet((xCenter - xSize), (yCenter - ySize), (zCenter + zSize), 0.0f);
	m_working_cuboidvertices[4] = XMVectorSet((xCenter + xSize), (yCenter + ySize), (zCenter - zSize), 0.0f);
	m_working_cuboidvertices[5] = XMVectorSet((xCenter + xSize), (yCenter - ySize), (zCenter + zSize), 0.0f);
	m_working_cuboidvertices[6] = XMVectorSet((xCenter - xSize), (yCenter + ySize), (zCenter + zSize), 0.0f);
	m_working_cuboidvertices[7] = XMVectorSet((xCenter + xSize), (yCenter + ySize), (zCenter + zSize), 0.0f);
...
bool ViewFrustrum::CheckRectangle(const FXMMATRIX world, float xsize, float ysize, float zsize)
{
	...
	m_working_cuboidvertices[0] = XMVector2TransformCoord(XMVectorSet(-xsize, -ysize, -zsize, 0.0f), world);
	m_working_cuboidvertices[1] = XMVector2TransformCoord(XMVectorSet(xsize, -ysize, -zsize, 0.0f), world);
	m_working_cuboidvertices[2] = XMVector2TransformCoord(XMVectorSet(-xsize, ysize, -zsize, 0.0f), world);
	m_working_cuboidvertices[3] = XMVector2TransformCoord(XMVectorSet(-xsize, -ysize, zsize, 0.0f), world);
	m_working_cuboidvertices[4] = XMVector2TransformCoord(XMVectorSet(xsize, ysize, -zsize, 0.0f), world);
	m_working_cuboidvertices[5] = XMVector2TransformCoord(XMVectorSet(xsize, -ysize, zsize, 0.0f), world);
	m_working_cuboidvertices[6] = XMVector2TransformCoord(XMVectorSet(-xsize, ysize, zsize, 0.0f), world);
	m_working_cuboidvertices[7] = XMVector2TransformCoord(XMVectorSet(xsize, ysize, zsize, 0.0f), world);
...
bool ViewFrustrum::CheckRectangleVertices(void)
{
	...
	for(int i = 0; i < 6; i++)
	{
		if(XMVector2GreaterOrEqual(XMPlaneDotCoord(m_planes[i], m_working_cuboidvertices[0]), NULL_VECTOR2))
			continue;

		if(XMVector2GreaterOrEqual(XMPlaneDotCoord(m_planes[i], m_working_cuboidvertices[1]), NULL_VECTOR2))
			continue;

		if(XMVector2GreaterOrEqual(XMPlaneDotCoord(m_planes[i], m_working_cuboidvertices[2]), NULL_VECTOR2))
			continue;

		if(XMVector2GreaterOrEqual(XMPlaneDotCoord(m_planes[i], m_working_cuboidvertices[3]), NULL_VECTOR2))
			continue;

		if(XMVector2GreaterOrEqual(XMPlaneDotCoord(m_planes[i], m_working_cuboidvertices[4]), NULL_VECTOR2))
			continue;

		if(XMVector2GreaterOrEqual(XMPlaneDotCoord(m_planes[i], m_working_cuboidvertices[5]), NULL_VECTOR2))
			continue;

		if(XMVector2GreaterOrEqual(XMPlaneDotCoord(m_planes[i], m_working_cuboidvertices[6]), NULL_VECTOR2))
			continue;

		if(XMVector2GreaterOrEqual(XMPlaneDotCoord(m_planes[i], m_working_cuboidvertices[7]), NULL_VECTOR2))
			continue;
...
bool ViewFrustrum::CheckRectangleVertices(const XMVECTOR rectverts[8])
{
	// Check if any of the planes of the rectangle are inside the view frustum.
	for(int i = 0; i < 6; i++)
	{
		if (XMVector2GreaterOrEqual(XMPlaneDotCoord(m_planes[i], rectverts[0]), NULL_VECTOR2))
			continue;

		if (XMVector2GreaterOrEqual(XMPlaneDotCoord(m_planes[i], rectverts[1]), NULL_VECTOR2))
			continue;

		if (XMVector2GreaterOrEqual(XMPlaneDotCoord(m_planes[i], rectverts[2]), NULL_VECTOR2))
			continue;

		if (XMVector2GreaterOrEqual(XMPlaneDotCoord(m_planes[i], rectverts[3]), NULL_VECTOR2))
			continue;

		if (XMVector2GreaterOrEqual(XMPlaneDotCoord(m_planes[i], rectverts[4]), NULL_VECTOR2))
			continue;

		if (XMVector2GreaterOrEqual(XMPlaneDotCoord(m_planes[i], rectverts[5]), NULL_VECTOR2))
			continue;

		if (XMVector2GreaterOrEqual(XMPlaneDotCoord(m_planes[i], rectverts[6]), NULL_VECTOR2))
			continue;

		if (XMVector2GreaterOrEqual(XMPlaneDotCoord(m_planes[i], rectverts[7]), NULL_VECTOR2))
			continue;
