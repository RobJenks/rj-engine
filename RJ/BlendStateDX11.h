...
public:
	...
	*/
	...
	void SetBlendMode(const BlendMode& blendMode, TBlendModeArray::size_type index);
	void SetBlendModes(const TBlendModeArray & blendModes);
	const TBlendModeArray & GetBlendModes() const;

	/**
	* Set a constant blend factor that is used when the BlendFactor is set to ConstBlendFactor.
	*/
	void SetConstBlendFactor(const XMFLOAT4 & constantBlendFactor);
	const XMFLOAT4& GetConstBlendFactor() const;
...
private:
	...
	TBlendModeArray								m_blendmodes;

	bool										m_alphaToCoverageEnabled;
	bool										m_independentBlendEnabled;
	uint32_t									m_samplemask;

	XMFLOAT4									m_constblendfactor;
