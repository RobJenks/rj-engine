...
public:
	...
	CMPINLINE D3DXVECTOR3				GetBaseMeshSize(void) const		{ return m_meshsize; }

	// Retrieve the actual model size (equivalent to BaseMeshSize * ScaleFactor)
	CMPINLINE D3DXVECTOR3				GetModelSize(void) const		{ return m_modelsize; }
	...
	void								SetModelSize(D3DXVECTOR3 size);
	...
	void								SetViewOffsetPercentage(const D3DXVECTOR3 & offset_pc);

	// Retrieve the view offset as an absolute translation, or as a percentage of total model size
	CMPINLINE D3DXVECTOR3				GetViewOffset(void) const				{ return m_absoluteviewoffset; }
	CMPINLINE D3DXVECTOR3				GetViewOffsetPercentage(void) const		{ return m_viewoffset; }
...
protected:
	...
	D3DXVECTOR3							m_viewoffset;				// Percentage of model size
	D3DXVECTOR3							m_absoluteviewoffset;		// Absolute offset (read-only, derived)
	...
	CMPINLINE void						SetScaleFactor(D3DXVECTOR3 sf)	{ SetScaleFactor(XMFLOAT3(sf.x, sf.y, sf.z)); }
