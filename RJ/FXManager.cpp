...
NormalMapEffect::NormalMapEffect(ID3D11Device* device, const std::string& filename)
	: Effect(device, filename)
{
	Light1Tech    = mFX->GetTechniqueByName("Light1");
	Light2Tech    = mFX->GetTechniqueByName("Light2");
...
BuildShadowMapEffect::BuildShadowMapEffect(ID3D11Device* device, const std::string& filename)
	: Effect(device, filename)
{
...
SsaoNormalDepthEffect::SsaoNormalDepthEffect(ID3D11Device* device, const std::string& filename)
	: Effect(device, filename)
{
...
SsaoEffect::SsaoEffect(ID3D11Device* device, const std::string& filename)
	: Effect(device, filename)
{
...
SsaoBlurEffect::SsaoBlurEffect(ID3D11Device* device, const std::string& filename)
	: Effect(device, filename)
{
...
DebugTexEffect::DebugTexEffect(ID3D11Device* device, const std::string& filename)
	: Effect(device, filename)
{
...
#pragma region FXManager

FXManager::FXManager(DXLocaliser *localiser)
{
	// Store a reference to the DX localiser
	m_locale = localiser;

	// Initialise variables to default values on construction
	BasicFX = NULL;
	NormalMapFX = NULL;
}

Result FXManager::Initialise(ID3D11Device *device)
{
	// Initialise each effect in turn
	BasicFX           = new BasicEffect(device, BuildStrFilename(D::DATA, "Shaders/fx_basic.fx"));
	NormalMapFX       = new NormalMapEffect(device, BuildStrFilename(D::DATA, "Shaders/fx_normalmap.fx"));
