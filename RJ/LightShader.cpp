...
LightShader::LightShader(const DXLocaliser *locale)
{
...
Result LightShader::Initialise(ID3D11Device* device, HWND hwnd)
{
	...
	if (m_locale->Locale.ShaderModelLevel == DXLocaliser::SMLevel::SM_5_0)
	{
		...
		result = InitialiseShader_SM5(device, hwnd, ShaderFilename("light_sm_5_0.vs").c_str(), 
													ShaderFilename("light_sm_5_0.ps").c_str());
	}
	else
	{
		// Otherwise, for all other versions initialise a Shader Model 2 shader
		result = InitialiseShader_SM2(device, hwnd, ShaderFilename("light_sm_2_0.vs").c_str(),
													ShaderFilename("light_sm_2_0.ps").c_str());
	}
...
Result LightShader::Render(ID3D11DeviceContext *deviceContext, UINT vertexCount, UINT indexCount, UINT instanceCount,
...
Result LightShader::SetLightParameters(D3DXVECTOR3 lightDirection, D3DXVECTOR4 ambientColor, D3DXVECTOR4 diffuseColor)
{
...
Result LightShader::InitialiseShader_SM5(ID3D11Device* device, HWND hwnd, const char* vsFilename, const char* psFilename)
{
	HRESULT result;
...
Result LightShader::InitialiseShader_SM2(ID3D11Device* device, HWND hwnd, const char* vsFilename, const char* psFilename)
{
	HRESULT result;
	ID3D10Blob* errorMessage;
	ID3D10Blob* vertexShaderBuffer;
...
void LightShader::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, const char* shaderFilename)
{
	char* compileErrors;
	SIZE_T bufferSize, i;
	ofstream fout;
...
Result LightShader::SetShaderParameters(ID3D11DeviceContext *deviceContext, D3DXMATRIX viewMatrix, D3DXMATRIX projectionMatrix, 
										ID3D11ShaderResourceView* texture)
{
	...
	if(FAILED(result))
	{
		return ErrorCodes::CouldNotObtainLightShaderBufferLock;
	}
...
void LightShader::RenderShader(ID3D11DeviceContext *deviceContext, UINT vertexCount, UINT indexCount, UINT instanceCount)
{
