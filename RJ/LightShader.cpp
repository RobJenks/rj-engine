...
Result LightShader::Initialise(Rendering::RenderDeviceType * device, HWND hwnd)
{
	...
	result = InitialiseVertexShader(device, ShaderFilename("light.vs.cso"));
	if (result != ErrorCodes::NoError) return result;

	result = InitialisePixelShader(device, ShaderFilename("light.ps.cso"));
	if (result != ErrorCodes::NoError) return result;
...
Result LightShader::InitialiseVertexShader(Rendering::RenderDeviceType  *device, std::string filename)
{
	...
	if (result != ErrorCodes::NoError) return ErrorCodes::ErrorCreatingLightShaderVS;
	...
	if (result != ErrorCodes::NoError) return ErrorCodes::ErrorCreatingLightShaderConstBuffersVS;
...
Result LightShader::InitialisePixelShader(Rendering::RenderDeviceType  *device, std::string filename)
{
	...
	// Parameter check
	if (!device || filename == NullString) return ErrorCodes::NullInputToCreateLightShaderPS;

	// Attempt to load and create the compiled shader 
	result = ShaderManager::CreatePixelShader(device, filename, &m_pixelShader);
	if (result != ErrorCodes::NoError) return ErrorCodes::ErrorCreatingLightShaderPS;
...
Result XM_CALLCONV LightShader::Render(Rendering::RenderDeviceContextType  *deviceContext, UINT vertexCount, UINT indexCount, UINT instanceCount,
