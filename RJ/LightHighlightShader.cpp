...
LightHighlightShader::LightHighlightShader(void)
{
...
Result LightHighlightShader::Initialise(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitialiseVertexShader(device, ShaderFilename("light_highlight.vs.cso"));
	if (result != ErrorCodes::NoError) return result;

	result = InitialisePixelShader(device, ShaderFilename("light_highlight.ps.cso"));
	if (result != ErrorCodes::NoError) return result;
...
// Initialise shader
Result LightHighlightShader::InitialiseVertexShader(ID3D11Device *device, std::string filename)
{
	Result result;

	// Parameter check
	if (!device || filename == NullString) return ErrorCodes::NullInputToCreateLightHighlightShaderVS;
	...
	if (result != ErrorCodes::NoError) return ErrorCodes::ErrorCreatingLightHighlightShaderVS;
	...
	if (result != ErrorCodes::NoError) return ErrorCodes::ErrorCreatingLightHighlightShaderConstBuffersVS;
...
// Initialise shader
Result LightHighlightShader::InitialisePixelShader(ID3D11Device *device, std::string filename)
{
	Result result;

	// Parameter check
	if (!device || filename == NullString) return ErrorCodes::NullInputToCreateLightHighlightShaderPS;

	// Attempt to load and create the compiled shader 
	result = ShaderManager::CreatePixelShader(device, filename, &m_pixelShader);
	if (result != ErrorCodes::NoError) return ErrorCodes::ErrorCreatingLightHighlightShaderPS;
	...
	if (result != ErrorCodes::NoError) return ErrorCodes::ErrorCreatingLightHighlightShaderSamplerPS;

	// Create the shader constant buffer
	result = ShaderManager::CreateStandardDynamicConstantBuffer(sizeof(PSBufferType), device, &m_cbuffer_ps);
	if (result != ErrorCodes::NoError) return ErrorCodes::ErrorCreatingLightHighlightShaderConstBuffersPS;
