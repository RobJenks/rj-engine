////////////////////////////////////////////////////////////////////////////////
// Filename: LightHighlightShader.cpp
////////////////////////////////////////////////////////////////////////////////
...
LightHighlightShader::LightHighlightShader(const DXLocaliser *locale)
{
...
LightHighlightShader::LightHighlightShader(const LightHighlightShader& other)
{
}


LightHighlightShader::~LightHighlightShader()
{
}


Result LightHighlightShader::Initialise(ID3D11Device* device, HWND hwnd)
{
	Result result;
...
void LightHighlightShader::Shutdown()
{
...
Result XM_CALLCONV LightHighlightShader::Render(ID3D11DeviceContext *deviceContext, UINT vertexCount, UINT indexCount, UINT instanceCount,
...
Result LightHighlightShader::SetLightParameters(XMFLOAT3 lightDirection, XMFLOAT4 ambientColor, XMFLOAT4 diffuseColor)
{
...
Result LightHighlightShader::InitialiseShader_SM5(ID3D11Device* device, HWND hwnd, const char* vsFilename, const char* psFilename)
{
	HRESULT result;
	...
	if(FAILED(result))
	{
		...
		if(errorMessage)
		{
			...
			return ErrorCodes::LightHighlightPixelShaderCompilationFailed;
		}
		// If there was nothing in the error message then it simply could not find the file itself.
		else
		{
			return ErrorCodes::LightHighlightPixelShaderFileMissing;
		}
	}
	...
	if(FAILED(result))
	{
		return ErrorCodes::CannotCreateLightHighlightPixelShader;
	}
	...
	lightBufferDesc.ByteWidth = sizeof(LightHighlightBufferType);
...
Result LightHighlightShader::InitialiseShader_SM2(ID3D11Device* device, HWND hwnd, const char* vsFilename, const char* psFilename)
{
	HRESULT result;
	ID3D10Blob* errorMessage;
	ID3D10Blob* vertexShaderBuffer;
...
void LightHighlightShader::ShutdownShader()
{
...
void LightHighlightShader::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, const char* shaderFilename)
{
	char* compileErrors;
	SIZE_T bufferSize, i;
	ofstream fout;
...
Result XM_CALLCONV LightHighlightShader::SetShaderParameters(ID3D11DeviceContext *deviceContext, const FXMMATRIX viewMatrix, const CXMMATRIX projectionMatrix,
										ID3D11ShaderResourceView* texture)
{
	...
	LightHighlightBufferType* dataPtr2;
	...
	if(FAILED(result))
	{
		return ErrorCodes::CouldNotObtainLightHighlightShaderBufferLock;
	}

	// Get a pointer to the data in the constant buffer.
	dataPtr2 = (LightHighlightBufferType*)mappedResource.pData;
